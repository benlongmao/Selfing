...
**本仓库即 GitHub 上的 [`benlongmao/Selfing`](https://github.com/benlongmao/Selfing)，是 Selfing 的规范公开源码与英文优先主线**（内部也常称为 **S** / **`s-main`**；另有一套中文优先的 `s` 树由作者维护）。核心机制一致；UI、运行提示、模型侧字符串、embedding 默认与对外 README 面向英文读者。**`docs/` 设计文档以英文为主**；本文件为中文总览，细部请以英文 `docs/*.md` 为准。
