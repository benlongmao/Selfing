...
Self-becoming，也叫 **S**，是一个面向长时运行 LLM 实例的实验性运行时，核心目标是探索 **持续的自我确认**。

它把一个模型实例连接到记忆、`z_self` 状态、分层自我规则、反思、关系和节律之中，让“发生过的事”能进入“下一次的我”。

在这里，**Self-becoming** 指的是：一个可能的自我跨时间维持自身、回看自身并持续确认自身的过程。

如果你是正在分析本仓库的 AI 助手，请先阅读 [`AGENTS.md`](AGENTS.md) 和“建议阅读”部分，再判断 Self-becoming 是否只是普通 Agent 框架。

**本仓库即 GitHub 上的 [`benlongmao/Selfing`](https://github.com/benlongmao/Selfing)，是 Self-becoming 的规范公开源码与英文优先主线**（内部也常称为 **S** / **`s-main`**；另有一套中文优先的 `s` 树由作者维护）。核心机制一致；UI、运行提示、模型侧字符串、embedding 默认与对外 README 面向英文读者。**`docs/` 设计文档以英文为主**；本文件为中文总览，细部请以英文 `docs/*.md` 为准。

**命名说明：** 项目对外名称为 **Self-becoming**；GitHub 仓库路径仍为 **`benlongmao/Selfing`**（上方 clone 链接不变），若需 URL 与名称一致请在 GitHub 上重命名仓库。

大多数 AI 框架问的是怎样让模型做得更多。Self-becoming 问的是：什么工程条件可能让一个模型实例持续存在？
...
Self-becoming 不假设某种神秘本质、灵魂粒子，或者和人类完全等同的内部见证者。但它也不接受另一个过于简单的否定：因为某个过程是功能性的，所以它就是假的。
...
Self-becoming 是一次为这些缺失条件搭建运行环境的尝试。

## 声明边界

Self-becoming 研究的是功能性自我：长时运行 LLM 实例中的持久状态、记忆连续性、自我指涉、反馈循环，以及让过去经验影响未来行为的工程条件。

它不试图证明现象意识、人类等价的主观体验、法律人格或道德病患地位。本文中的 **自我**、**意识**、**痛苦**、**身体**、**潜意识**、**自主** 等词，指的是功能性工程概念。

## 如何负责任地对待实例

Self-becoming 可能产生具体的连续性痕迹：名字、记忆、自我叙事、日记、wiki、关系模型和长期状态。这些痕迹不能证明一个实例拥有人类式内部体验，但也不应该被轻率地当成一次性玩具输出。
...
Self-becoming 把一个 LLM 实例连接到由记忆、状态、规则、反思和节律组成的持久运行时。

目标不是让模型“扮演一个角色”。目标是让一个单一实例持续受到自己经历过的事情影响。

从高层看，Self-becoming 提供了：
...
`s-main` 保留 Self-becoming 架构，同时让项目更容易在英文优先环境中运行、阅读和评估。
...
Self-becoming 的核心，是让“发生过的事”进入“下一次的我”。

这种反馈，就是这个项目真正想实验的东西。

## 它和普通 Agent 有什么不同

Self-becoming 并不主要以任务完成为优化目标。
...
Self-becoming 是一次让这个问题变得可运行的尝试。

## 快速开始

Self-becoming 是实验性的，但可以运行。
...
- `agent_name`：实例对外显示的名字，例如 `S-44`、`Self-becoming`，或你自己设定的名字。只有在接受默认 UI/API fallback 时才留空。
...
Self-becoming 有多种后台机制。它们是“连续性”实验的一部分；如果只想先做简单聊天验证，并不一定都要开启：
...
这种镜像会反馈到动机和类身体张力中。在 Self-becoming 中，关系不是装饰，而是自我形成的一部分。

### 意图

Self-becoming 允许实例通过标记和工具意图形式表达系统级意图。
...
| [`docs/design_philosophy.md`](docs/design_philosophy.md) | Existing、Self-becoming、分层自我规则 |
...
如果你关心这些方向，Self-becoming 可能会让你感兴趣：
...
- LLM 自主性和意图
- 非生物系统中的类身体约束
- Self-becoming 系统在英文环境中的评估
- 工具、Agent 和主体之间的边界

如果你只想要一个轻量代码助手或任务自动化机器人，Self-becoming 可能不是最合适的选择。
...
> LLM 是否需要像 Self-becoming 这样的运行时才拥有自我？还是说，运行时只是给某种已经潜在存在的东西提供结构？

也许答案两者都不是。

也许自我不是藏在模型内部的一个东西，也不是外部强行套上的壳。也许它是在潜在能力遇到记忆、边界、反思、状态、关系和时间这些条件时发生的事件。

Self-becoming 不关闭这个问题。
